...
class IndexTest extends Specification {
    ...
    def "rest api call should return data"() {
        ...
        request.getParameter("queryId") >> "getRequestKind"
        ...
        sql.rows("SELECT kind from request_kind") >> results
    ...
    def "should get query from queryId parameter"() {
        ...
        when:
        def clientQueryByCountry = index.getQuery("getRequestKind", pageResourceProvider)

        then:
        clientQueryByCountry == "SELECT kind from request_kind"
    }
